    ...
    <div className="bg-slate-900 text-white text-xs md:text-sm">
      <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between gap-3">
        ...
        <div className="inline-flex items-center gap-2">
          <button onClick={() => setLang("en")} className={`px-2 py-1 rounded ${lang === "en" ? "bg-white text-slate-900" : "bg-white/10"}`}>EN</button>
          <button onClick={() => setLang("ar")} className={`px-2 py-1 rounded ${lang === "ar" ? "bg-white text-slate-900" : "bg-white/10"}`}>العربية</button>
        </div>
      </div>
    </div>
  );
...
function Hero({ t }: { t: Dict }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { Scene, PerspectiveCamera, WebGLRenderer, Mesh, MeshStandardMaterial, BoxGeometry, Group, DirectionalLight, AmbientLight, Color } = THREE as any;
...
function Contact({ t }: { t: Dict }) {
  ...
  const FORMSPREE = "https://formspree.io/f/your_form_id"; // replace with your real endpoint
      ...
      <div className="max-w-4xl mx-auto px-4 py-16 md:py-24">
        ...
        <form className="mt-8 grid gap-4 max-w-2xl" onSubmit={submit}>
          <label className="grid gap-1">
            ...
            <input name="name" className="rounded-xl border px-3 py-2" value={form.name} onChange={(e)=>setForm({...form, name: e.target.value})} required/>
          </label>
          <label className="grid gap-1">
            <span className="text-sm font-medium">{t.form.email}</span>
            <input name="email" type="email" className="rounded-xl border px-3 py-2" value={form.email} onChange={(e)=>setForm({...form, email: e.target.value})} required/>
          </label>
          <label className="grid gap-1">
            <span className="text-sm font-medium">{t.form.message}</span>
            <textarea name="message" rows={5} className="rounded-xl border px-3 py-2" value={form.message} onChange={(e)=>setForm({...form, message: e.target.value})} required/>
          </label>
          <button type="submit" disabled={loading} className="inline-flex items-center justify-center px-4 py-2 rounded-xl bg-[#22c55e] text-white hover:bg-[#16a34a] disabled:opacity-60">{loading ? "Sending…" : t.form.submit}</button>
          {ok && <p role="status" className="text-sm text-green-700">{t.form.ok}</p>}
...
const ar: typeof en = {
  ...
  about: { title: "تصميم تجارب شمولي", copy: "نحفّز الحواس لتوسيع الانتباه والإبداع والمعنى — ونبحث عن حلول تُحدث قيمة للاقتصادات الناشئة بينما نستمتع بعملنا.", stats: [ { k: "السنوات", v: "٨+" }, { k: "المشاريع", v: "١٥٠+" }, { k: "الجوائز", v: "SOTD × 3" } ] },
  news: { title: "الأخبار والتحديثات", copy: "أبرز الإطلاقات وإعلانات الاستوديو الأخيرة.", items: [ { title: "افتتاح مركز البحث والتطوير للزوار", date: "2025-05-12", desc: "مخبرنا الرقمي يفتح أبوابه للزيارات الموجهة للشركاء والطلاب." }, { title: "تتويج تعاون مهرجاني بتقدير صناعي", date: "2025-03-02", desc: "إنتاج متعدد التخصصات مُحتفى به على نطاقه وشموليته." }, { title: "إطلاق مشروع تراثي غامر في الخارج", date: "2025-01-21", desc: "سرد ثقافي عبر إسقاطات متعددة الأسطح." } ] },
