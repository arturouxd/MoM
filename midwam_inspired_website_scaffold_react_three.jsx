...
// Midwam-inspired scaffold: dark hero with WebGL, strong type, bilingual toggle (EN/AR).
// Original build for Made of Magic — not a clone. Replace any placeholder copy/assets as you like.
...
export default function MidwamInspiredSite() {
  ...
  // Inject Google Fonts (Inter body, Outfit headings)
  useEffect(() => {
    const href =
        ...
        <div className="h-16 flex items-center justify-between">
          <a href="#home" className="flex items-center gap-2 font-semibold">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
              <path d="M12 2l9 4.5v10L12 22l-9-5.5v-10L12 2z" />
            </svg>
            <span>Made of Magic</span>
          </a>
          <nav className="hidden md:flex items-center gap-6 text-sm">
            <a href="#experiences" className="hover:text-slate-600">
              {t.nav.experiences}
            </a>
            ...
            <a
              href="#contact"
              className="inline-flex items-center px-3 py-2 rounded-xl bg-[#22c55e] text-white hover:bg-[#16a34a]"
            >
              {t.nav.cta}
            </a>
          </nav>
          <button
            onClick={() => setOpen((v) => !v)}
            className="md:hidden w-10 h-10 rounded-xl border inline-flex items-center justify-center"
            aria-label="Menu"
          >
...
function Hero({ t }: { t: Dict }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // === Lightweight “M” glyph (4 bars) ===
      const material = new MeshStandardMaterial({ metalness: 0.7, roughness: 0.2, color: 0xffffff });
      const logo = new Group();
      const makeBar = (w: number, h: number, d: number, x: number, y: number, rz = 0) => {
      ...
      // Left vertical, right vertical, two diagonals:
      makeBar(0.25, 2.6, 0.25, -1.6, 0, 0);
      makeBar(0.25, 2.6, 0.25, 1.6, 0, 0);
      makeBar(0.25, 2.6, 0.25, -0.6, 0, -Math.PI / 4);
      makeBar(0.25, 2.6, 0.25, 0.6, 0, Math.PI / 4);
      scene.add(logo);
  ...
  return (
    <section id="home" className="relative overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 py-20 md:py-28 grid md:grid-cols-2 items-center gap-10">
        <div>
          <h1
            className="text-4xl md:text-6xl font-bold tracking-tight text-white font-[Outfit,Inter,sans-serif]"
            data-reveal
          >
            {t.hero.title}
          </h1>
          ...
          <div className="mt-6 flex flex-wrap gap-3" data-reveal>
            <a
              href="#experiences"
              className="inline-flex items-center px-4 py-2 rounded-xl bg-[#22c55e] text-white hover:bg-[#16a34a]"
            >
              {t.hero.ctaPrimary}
            </a>
            <a
              href="#contact"
              className="inline-flex items-center px-4 py-2 rounded-xl border border-white/30 text-white hover:bg-white/10"
            >
              {t.hero.ctaSecondary}
            </a>
          </div>
  ...
  return (
    <section id="experiences" className="bg-white border-t">
      <div className="max-w-7xl mx-auto px-4 py-16 md:py-24">
        <h2 className="text-2xl md:text-3xl font-semibold tracking-tight font-[Outfit,Inter,sans-serif]">
          {t.experiences.title}
        </h2>
...
function Contact({ t }: { t: Dict }) {
  ...
  // Replace with your real Formspree endpoint, e.g. "https://formspree.io/f/xxxxxx"
  ...
  return (
    <section id="contact" className="bg-slate-50 border-t">
      <div className="max-w-4xl mx-auto px-4 py-16 md:py-24">
        ...
        <form className="mt-8 grid gap-4 max-w-2xl" onSubmit={submit}>
          ...
          <button
            type="submit"
            disabled={loading}
            className="inline-flex items-center justify-center px-4 py-2 rounded-xl bg-[#22c55e] text-white hover:bg-[#16a34a] disabled:opacity-60"
          >
...
function SiteFooter({ t }: { t: Dict }) {
  return (
    <footer className="border-t">
      <div className="max-w-7xl mx-auto px-4 py-12 grid md:grid-cols-3 gap-6 text-sm">
        ...
        <div>
          ...
          <ul className="mt-2 grid gap-2 text-slate-600">
            ...
            <li>
              <a href="#experiences" className="hover:text-slate-900">
                {t.nav.experiences}
              </a>
            </li>
            <li>
      ...
      <div className="border-t text-xs text-slate-500 py-6 text-center">
        © {new Date().getFullYear()} Made of Magic. {t.footer.rights}
      </div>
    </footer>
  );
}
...
const en = {
    ...
    items: [
      {
        ...
        desc:
          "Assessment, competitive scan, portfolio architecture, and foundational brand system (mission, vision, positioning, voice, tone, taglines). Executive alignment workshops to define Brand DNA & values; investor-ready narrative and unification roadmap.",
        meta: "Deliverable: Brand Strategy Playbook"
      },
      {
        title: "UX & Journey Architecture",
        desc:
          "Cross-brand UX audits, information architecture restructuring, persona-based journeys (Healthcare, SMB, Imaging, Lease→Build), content and messaging direction, and conversion opportunities.",
        meta: "Deliverable: UX Architecture Toolkit"
      },
      {
        title: "Integration & Roadmap Layer",
        desc:
          "Cross-brand connectivity concepts and shared design-system guardrails (voice, tone, UX patterns). Tactical quick wins now, with a 5-year brand-unification roadmap.",
        meta: "Deliverable: Integration & Roadmap Deck"
      },
      {
        title: "AI-Ready Brand & UX Spec",
        desc:
          "Machine-readable brand rules (taxonomy, metadata, component specs), voice & tone matrices for emotional AI, structured content schema (FAQ/HowTo/Product) and JSON journeys/site maps for direct AI ingest.",
        meta: "Deliverable: AI-Ready Spec Pack"
      }
    ]
  },
  about: {
    ...
    copy:
      "Made of Magic builds the backbone of your brand: a durable source of truth that aligns teams, simplifies decisions, and plugs directly into AI assistants and tools.",
  ...
  footer: {
    copy: "We create unified, future-proofed brands — one source of truth for your teams and their AI.",
    links: "Links",
    contact: "Contact",
